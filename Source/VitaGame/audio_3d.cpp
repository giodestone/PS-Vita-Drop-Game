...
{
	emitter->set_added_to_audio_3d(true);
...
//	For each emitter
	for (auto emitter = emitters_.begin(); emitter != emitters_.end();)
	{
...
//		If distance is less than emitter range
		...
		if (distance < (*emitter)->radius())
		{
			...
			if ((*emitter)->playing())
			{
				if (!audio_manager_->sample_voice_playing((*emitter)->voice_id()) || (*emitter)->to_be_deleted() )
				{
